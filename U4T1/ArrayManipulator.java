public class ArrayManipulator {
   ...
   * Example: if ints[] is {4, 5, 6, 10}, the indices are 0, 1, 2, 3, and two
               middle indices are 1 and 2, so return 1 (the earlier of the two)
   */
  ...
  /** Returns the average, as a double, of the average of the first, last, an
  ...
  public double getNumsAverage() {
    double average = (nums[0] + getNumsMiddleIndex() + nums[nums.length - 1]) / 3.0;
    return average;
  }

  /** Returns true if both the nums and words arrays have the same number o
